...
import { getNews, getNewsPage } from '../services/api/cms-api/news';
...
export async function getStaticProps() {
  const secondsToFirstRefresh = 60 * 60 * 6; // 6 hrs
  const cmsNews = await getNews(3);
  const cmsPage = await getNewsPage(articleId);
  ...
  return {
    ...
    // - At most once every secondsToFirstRefresh seconds
    revalidate: secondsToFirstRefresh // In seconds
  };
}

interface Props
  extends TranslationProps,
              ...
              {modules.slice(0, 2).map((module: any, index: number) => (
                <InfoBox
                  ...
                  {...(isAbsoluteUrl(
                    ...
                    : {
                        href: module?.field_link?.uri?.replace('internal:', '')
                      })}
                >
                  <InfoBoxIcon>
                    {index === 0 ? (
...
export default compose<FC>(memo, withTranslations)(MainPage);
